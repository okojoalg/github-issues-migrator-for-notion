...
class Notion:
    ...
    def __init__(self, github: GitHub, page_id: str, token: str, time_zone: str):
        self.github = github
    ...
    def post(self, issue, comments):
        ...
        properties = {
            ...
            "type": {"select": {"name": "Pull Request" if pull_request else "Issue"}},
            ...
            "ref": {"url": pull_request.get("html_url") if pull_request else issue.get("html_url")},
            "created_at": {"date": {"start": issue.get("created_at"), "time_zone": self.time_zone}},
            "updated_at": {"date": {"start": issue.get("updated_at"), "time_zone": self.time_zone}},
        }
        if issue.get("closed_at"):
            properties["closed_at"] = {"date": {"start": issue.get("closed_at"), "time_zone": self.time_zone}}
        children = []
            ...
            for comment in comments:
                ...
                if body := comment.get("body"):
                    ...
                    children += [self.append_block(
                        ...
                        f'created_at: {datetime.fromisoformat(comment.get("created_at")).astimezone(self.time_zone_obj).isoformat()}, '
                        f'updated_at: {datetime.fromisoformat(comment.get("updated_at")).astimezone(self.time_zone_obj).isoformat()}'
                    )]
        payload = {
